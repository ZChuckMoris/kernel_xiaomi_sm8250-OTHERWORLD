...
TARGET_ARCH="arm64"
[[ -f "$KERNEL_ROOT_DIR/arch/$TARGET_ARCH/configs/vendor/apollo_defconfig" ]] && \
KERNEL_CONFIG="apollo_defconfig" \
|| KERNEL_CONFIG="apollo_defconfig"
ZIP_KERNEL_STR="coreLinux"
ZIP_DEVICE_NAME="apollo"
ZIP_PREFIX_STR="OtherWorldKernel-v4.3.62-MIUI-PlusEdition-APOLLO"
...
build() {
	...
	run "PATH=$PATH CROSS_COMPILE=$CROSS_COMPILE CROSS_COMPILE_ARM32=$CROSS_COMPILE_ARM32 make $(echo -e $MAKEOPTS) O=out ARCH=$TARGET_ARCH CC=\"${NEW_CC}\" -j8 || exit"
